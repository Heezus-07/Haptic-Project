function structureParsedMultipartData(rawData) {
	const result = {};

	rawData?.forEach(entry => {
		let dataToAdd;

		if (entry.filename) dataToAdd = {
			filename: entry.filename,
			type: entry.type,
			data: entry.data
		};
		else dataToAdd = entry.data.toString('utf-8');

		if (result.hasOwnProperty(entry.name)) {
			if (Array.isArray(result[entry.name])) result[entry.name].push(dataToAdd);
			else result[entry.name] = [result[entry.name], dataToAdd];
		} else result[entry.name] = dataToAdd;
	});

	return result;
}

// The following code was copied from https://github.com/nachomazzara/parse-multipart-data/blob/master/src/multipart.ts
// This was done for simplicity and to not have any installed packages
// It should be changed to a package like formidable or multer later on.
const ParsingState = {
	INIT: 0,
	READING_HEADERS: 1,
	READING_DATA: 2,
